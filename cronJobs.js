...
// Schedule the cron job to run daily at 3:10 AM UTC (11:10 AM UTC+8)
cron.schedule("10 3 * * *", async () => {
  // 3:10 AM UTC is 11:10 AM UTC+8
  const now = new Date();
  const philippineTime = new Date(
    now.toLocaleString("en-US", { timeZone: "Asia/Manila" })
  );
  if (philippineTime.getHours() === 11 && philippineTime.getMinutes() === 10) {
